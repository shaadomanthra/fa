...
<form class="bg-light mt-4 rounded border p-3" action="{{ route('test.instructions',$test->slug)}}" method="get">
  <div class="row">
    <div class="col-5">
      <input type="text" class="form-control" name="name" placeholder="Enter your full name" value="{{ (old('name')) ? old('name') : '' }}">
    </div>
    <div class="col-5">
      <input type="text" class="form-control" name="phone" placeholder="Enter 10 digit phone number" value="{{ (old('phone')) ? old('phone') : '' }}">
    </div>
    <div class="col-2">
